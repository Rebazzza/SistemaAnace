    ...
    public boolean registrarVenta(String codigoVenta, int codigoEmpleado, Date fecha, double descuento,
        ...
        try {
            ...
            for (int i = 0; i < modelo.getRowCount(); i++) {
                ...
                String sqlStockActual = "SELECT STOCK FROM PRODUCTO WHERE CODIGO = ?";
                PreparedStatement psStock = conn.prepareStatement(sqlStockActual);
                psStock.setString(1, codProducto);
                ResultSet rsStock = psStock.executeQuery();
                int stockAnterior = 0;
                if (rsStock.next()) {
                    stockAnterior = rsStock.getInt("STOCK");
                }
                rsStock.close();
                psStock.close();
...
// Aquí ya estás actualizando el stock:
                psActualizarStock.setInt(1, cantidad);
                psActualizarStock.setString(2, codProducto);
                psActualizarStock.addBatch();

// 🔽 Insertar en KARDEX
                String sqlKardex = "INSERT INTO KARDEX (CODIGO_PRODUCTO, FECHA, TIPO_MOVIMIENTO, CANTIDAD, PRECIO_UNITARIO, STOCK_ANTERIOR, STOCK_ACTUAL, DESCRIPCION) "
                        + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                String referencia ="VENTA--Codigo Venta: "+codigoVenta+"/Codigo Empleado: "+codigoEmpleado;
                PreparedStatement psKardex = conn.prepareStatement(sqlKardex);
                psKardex.setString(1, codProducto);
                psKardex.setDate(2, new java.sql.Date(fecha.getTime()));
                psKardex.setString(3, "Salida"); // porque es una venta
                psKardex.setInt(4, cantidad);
                psKardex.setDouble(5, precioUnitario);
                psKardex.setInt(6, stockAnterior);
                psKardex.setInt(7, nuevoStock);
                psKardex.setString(8, referencia);
